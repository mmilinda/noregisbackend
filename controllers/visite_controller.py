...
async def enregistrer_entree(body: EntreeBody):
    ...
    visite_en_cours = await Visite.find_one(
        Visite.visiteur_id == oid,
        Visite.statut == "EN_COURS",
    )
    if visite_en_cours:
        raise HTTPException(
            status_code=409,
            detail="Ce visiteur est déjà à l'intérieur.",
        )
    ...
    heure_locale = datetime.now().strftime("%H:%M:%S")
    d = _visite_dict(visite)
    d["visiteur"] = visiteur.dict()

    return {
        "success": True,
        "message": f"Entrée enregistrée à {heure_locale}",
        "visite": d,
    }
...
async def enregistrer_sortie(visite_id: str):
    ...
    return {
        "success": True,
        "message": f"Sortie enregistrée. Durée : {duree_minutes} min.",
        "visite": d,
    }
