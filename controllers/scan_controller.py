...
def extraire_champ(
    ...
    labels_a_ignorer: Optional[re.Pattern] = None,
) -> Optional[str]:
...
def extraire_infos_piece(texte: str) -> dict:
    ...
    # ── Prénom
    prenom_brut = extraire_champ(
        lignes,
        re.compile(r"Pr[ée]noms?\s*[:\-]?|Given\s*names?\s*[:\-]?|Forenames?\s*[:\-]?", re.IGNORECASE),
        LABELS_REGEX,
    )
    if prenom_brut:
    ...
    # ── Nom
    nom_brut = extraire_champ(
        lignes,
        re.compile(r"^Nom\s*[:\-]?$|^Nom\s+[A-Z]|Surname\s*[:\-]?|Last\s*name\s*[:\-]?", re.IGNORECASE),
        LABELS_REGEX,
    )
    if nom_brut:
    ...
    if not infos["numero_piece"]:
        ...
        infos["numero_piece"] = next(
            (c for c in codes if re.search(r"[A-Z]", c) and re.search(r"[0-9]", c)), None
        )
...
async def scanner_image(request: Request, file: UploadFile):
    ...
    if sio:
        await sio.emit("ocr:donnees", {
            "infosExtraites": infos_extraites,
